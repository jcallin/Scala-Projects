...
case class SinglyLinkedList(head: Option[SingleNode])
    extends LinkedListIterable(head)
...
object SinglyLinkedList {
  ...
  def singlyLinkedListFromValues(values: List[AnyVal]): Option[SingleNode] = {
    @tailrec
    def singlyLinkedListFromValuesHelper(
        values: List[AnyVal],
        acc: Option[SingleNode]
    ): Option[SingleNode] = {
      values match {
        case Nil         => None
        case head :: Nil => Some(SingleNode(next = acc, value = head))
        case head :: tail =>
          singlyLinkedListFromValuesHelper(tail, Some(SingleNode(acc, head)))
      }
    }
