...
class UserDatabase(val users: List[User] = List.empty) extends StrictLogging {
  private var inboxes: Map[User, List[String]] = users.map(user => user -> List.empty[String]).toMap
  ...
  def mailTo(user: User, message: String): Boolean = {
    if (!users.contains(user)) {
      ...
      inboxes = inboxes.updatedWith(user)(
        emails => Some(message :: emails.getOrElse(List.empty[String])))
      true
    }
  }
